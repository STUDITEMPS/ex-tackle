defmodule Support do
  ...
  import Support.RabbitmqApi, only: [list_exchanges: 0, list_queues: 0]
  ...
  def rabbitmq_list_exchanges() do
    list_exchanges().body |> Enum.map(fn %{"name" => name} -> name end)
  end

  def rabbitmq_list_queues() do
    list_queues().body |> Enum.map(fn %{"name" => name} -> name end)
  end
